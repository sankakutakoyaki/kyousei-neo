...
IF OBJECT_ID(N'dbo.timework_edits', N'U') IS NOT NULL
    DROP TABLE dbo.timework_edits;
...
CREATE TABLE dbo.timework_edits (
    ...
    CONSTRAINT FK_neo_timework_edit_timework FOREIGN KEY (timework_id) REFERENCES dbo.timeworks(timework_id),
    CONSTRAINT CK_neo_timework_edit_has_value CHECK (edit_start_time IS NOT NULL OR edit_end_time IS NOT NULL)
);

CREATE UNIQUE INDEX UX_neo_timework_edit_active ON dbo.timework_edits(timework_id) WHERE state = 0;
CREATE INDEX IX_neo_timework_edit_history ON dbo.timework_edits(timework_id, timework_edit_id DESC);
