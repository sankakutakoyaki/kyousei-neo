...
export class AttachmentManager {
    ...
    bind() {
        ...
        this.fileInput?.addEventListener("change",()=>this.upload(this.fileInput.files));
        this.cameraInput?.addEventListener("change",()=>this.upload(this.cameraInput.files));
    }
    async open(parentId) {
    ...
    renderGroup(group) {
        ...
        drop.addEventListener("drop",e=>{e.preventDefault();drop.classList.remove("is-dragover");this.activeGroupId=group.attachmentGroupId;this.upload(e.dataTransfer.files);});
        body.append(drop);
    ...
    async upload(files){if(!this.activeGroupId||!files?.length)return;try{await AttachmentRepository.upload(this.parentType,this.parentId,this.activeGroupId,files);await this.refresh();}catch(e){DialogService.error(e.message);}}
