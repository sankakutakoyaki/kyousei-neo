...
export function init(area = document) {

    const groups = area.querySelectorAll("[data-enterfocus]");
...
// フォーカス遷移対象要素作成
export function createTabFocusElements() {
    // フォーカス遷移対象要素をフィルタリング
...
// フォーカス遷移対象要素フィルタリング
export function filterTabFocusElements(nodeList) {
    return Array.from(nodeList).filter(target => {
        ...
        // 正のtabindex属性を持つ場合は対象
        return target.hasAttribute("tabindex") && target.tabIndex >= 0;
    });
}
