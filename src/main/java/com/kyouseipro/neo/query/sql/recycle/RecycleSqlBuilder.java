...
public class RecycleSqlBuilder {
    ...
    private static String buildLogTableSql(String rowTableName) {
        return
            ...
            "  recycle_id INT, number NVARCHAR(255), maker_id INT, item_id INT, use_date DATE, delivery_date DATE, shipping_date DATE, loss_date DATE," +
            "  company_id INT, office_id INT, recycle_fee INT, disposal_site_id INT, version INT, state INT" +
            "); ";
    }
    ...
    private static String buildInsertLogSql(String rowTableName, String processName) {
        return
            ...
            "  number, maker_id, item_id, use_date, delivery_date, shipping_date, loss_date," +
            "  company_id, office_id, recycle_fee, disposal_site_id, version, state" +
            ") " +
    ...
    private static String buildOutputLogSql() {
        return
            "OUTPUT INSERTED.recycle_id, INSERTED.number, INSERTED.maker_id, INSERTED.item_id," +
    ...
    private static String baseSelectString() {
        return
            "SELECT r.recycle_id, r.number, r.maker_id, rm.name as maker_name, r.item_id, ri.name as item_name," +
            ...
            " r.recycle_fee, r.disposal_site_id, c2.name as disposal_site_name, r.version, r.state" +
            " r.version, r.state FROM recycles r" +
            ...
            " LEFT OUTER JOIN recycle_maker rm ON rm.recycle_maker_id = r.maker_id AND NOT (rm.state = ?)" +
            " LEFT OUTER JOIN recycle_item ir ON ri.recycle_item_id = r.item_id AND NOT (ri.state = ?)";
    }
